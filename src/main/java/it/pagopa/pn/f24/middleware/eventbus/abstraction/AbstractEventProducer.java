...
@Slf4j
@Component
public abstract class AbstractEventProducer<T extends PnF24GenericEvent<T>> implements Producer<T> {
    ...
    protected AbstractEventProducer(AmazonEventBridgeAsync amazonEventBridge, F24Config f24Config, String detailType, String name) {
    ...
    private String toJsonString(T detail) {
        ...
        amazonEventBridge.putEventsAsync(putEventsRequestBuilder(event),
                new AsyncHandler<>() {
                    ...
                    public void onError(Exception e) {
                        log.error("Send event with correlationId {} failed", event.getCxId(), e);
                    }

                    @Override
                    public void onSuccess(PutEventsRequest request, PutEventsResult putEventsResult) {
                        log.info("Event with correlationId {} sent successfully", event.getCxId());
